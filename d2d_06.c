...
void D2D_rajzolas(ID2D1HwndRenderTarget* pRT)
{
	...
	pRT->CreateSolidColorBrush(
		D2D1::ColorF(D2D1::ColorF::Black, 1.0f), &ecset);

	ID2D1StrokeStyle *vonalstilus;
	...
	pD2DFactory->CreateStrokeStyle(strokeStyleProperties, NULL, 0, &vonalstilus);
	...
	pRT->DrawRoundedRectangle(&roundedRect, ecset, 10.0f, vonalstilus);
	pRT->SetTransform(D2D1::Matrix3x2F::Translation(200, 10));
	pRT->FillRoundedRectangle(roundedRect, ecset);
